...
<c:forEach var="v" items="${datas}">
	...
	<h3>[${m.userid}] ${m.msg} &gt;&gt; [좋아요 ${m.favcount} | 댓글 ${m.replycount} | ${m.datetime}] <mytag:msg mid="${m.mid}" userid="${m.userid}" /></h3>
	...
	<ol><c:forEach  var="r" items="${v.rlist}">
		<li>${r.userid} >> ${r.rmsg} [${r.date}] <mytag:reply rid="${r.rid}" userid="${r.userid}" /> </li>
	</c:forEach></ol>
</c:forEach>
